...
public:
    ...
    ProjectDTO(const ProjectDTO &other)
    ...
    ProjectDTO &operator=(const ProjectDTO &other)
    {
        if (this != &other) {
            m_metaData = other.m_metaData;
            m_id = other.m_id;
    ...
    ProjectDTO &mergeWith(const ProjectDTO &other)
    {
        if (this != &other) {
    ...
    friend bool operator==(const ProjectDTO &lhs, const ProjectDTO &rhs);

    friend uint qHash(const ProjectDTO &dto, uint seed) noexcept;
...
Q_DECLARE_METATYPE(QleanyEditor::Contracts::DTO::Project::ProjectDTO)