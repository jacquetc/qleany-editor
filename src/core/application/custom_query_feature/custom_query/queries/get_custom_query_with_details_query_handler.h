...
{
class QLEANY_EDITOR_APPLICATION_CUSTOM_QUERY_EXPORT GetCustomQueryWithDetailsQueryHandler : public QObject
{
    Q_OBJECT
public:
    GetCustomQueryWithDetailsQueryHandler(InterfaceCustomQueryRepository *repository);
    Result<CustomQueryWithDetailsDTO> handle(QPromise<Result<void>> &progressPromise, const GetCustomQueryQuery &query);

private:
    InterfaceCustomQueryRepository *m_repository;
    Result<CustomQueryWithDetailsDTO> handleImpl(QPromise<Result<void>> &progressPromise, const GetCustomQueryQuery &query);
