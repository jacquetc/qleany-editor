...
{
class UpdateCustomQueryCommandValidator
{
public:
    UpdateCustomQueryCommandValidator(InterfaceCustomQueryRepository *customQueryRepository)
        : m_customQueryRepository(customQueryRepository)
    {
    }

    Result<void> validate(const UpdateCustomQueryDTO &dto) const

    {
        Result<bool> existsResult = m_customQueryRepository->exists(dto.id());
