...
class PresenterListModelRelationDTO
{
    Q_GADGET
...
public:
    ...
    enum RelationField { Undefined, Entity, DisplayedField, InRelationOf, RelationFieldName };
    Q_ENUM(RelationField);

    PresenterListModelRelationDTO()
        : m_id(0)
    ...
    PresenterListModelRelationDTO(int id, const RelationField &relationField, const QList<int> &relatedIds, int position)
        : m_id(id)
    ...
    PresenterListModelRelationDTO(const PresenterListModelRelationDTO &other)
    ...
    PresenterListModelRelationDTO &operator=(const PresenterListModelRelationDTO &other)
    {
        if (this != &other) {
            m_metaData = other.m_metaData;
            m_id = other.m_id;
    ...
    PresenterListModelRelationDTO &mergeWith(const PresenterListModelRelationDTO &other)
    {
        if (this != &other) {
    ...
    friend bool operator==(const PresenterListModelRelationDTO &lhs, const PresenterListModelRelationDTO &rhs);

    friend uint qHash(const PresenterListModelRelationDTO &dto, uint seed) noexcept;
...
inline bool operator==(const PresenterListModelRelationDTO &lhs, const PresenterListModelRelationDTO &rhs)
{
...
inline uint qHash(const PresenterListModelRelationDTO &dto, uint seed = 0) noexcept
...
Q_DECLARE_METATYPE(QleanyEditor::Contracts::DTO::PresenterListModel::PresenterListModelRelationDTO)