...
class CustomCommandRelationDTO
{
    Q_GADGET
...
public:
    ...
    enum RelationField { Undefined, Entities, DtoIn, DtoOut };
    Q_ENUM(RelationField);

    CustomCommandRelationDTO()
        : m_id(0)
    ...
    CustomCommandRelationDTO(int id, const RelationField &relationField, const QList<int> &relatedIds, int position)
        : m_id(id)
    ...
    CustomCommandRelationDTO(const CustomCommandRelationDTO &other)
    ...
    CustomCommandRelationDTO &operator=(const CustomCommandRelationDTO &other)
    {
        if (this != &other) {
            m_metaData = other.m_metaData;
            m_id = other.m_id;
    ...
    CustomCommandRelationDTO &mergeWith(const CustomCommandRelationDTO &other)
    {
        if (this != &other) {
    ...
    friend bool operator==(const CustomCommandRelationDTO &lhs, const CustomCommandRelationDTO &rhs);

    friend uint qHash(const CustomCommandRelationDTO &dto, uint seed) noexcept;
...
inline bool operator==(const CustomCommandRelationDTO &lhs, const CustomCommandRelationDTO &rhs)
{
...
inline uint qHash(const CustomCommandRelationDTO &dto, uint seed = 0) noexcept
...
Q_DECLARE_METATYPE(QleanyEditor::Contracts::DTO::CustomCommand::CustomCommandRelationDTO)