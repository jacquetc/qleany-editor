...
public:
    ...
    ApplicationComponentDTO(int id, const QUuid &uuid, const QDateTime &creationDate, const QDateTime &updateDate, const QString &commonCMakeFolderPath)
        : m_id(id)
        , m_uuid(uuid)
    ...
    ApplicationComponentDTO(const ApplicationComponentDTO &other)
    ...
    ApplicationComponentDTO &operator=(const ApplicationComponentDTO &other)
    {
        if (this != &other) {
            m_metaData = other.m_metaData;
            m_id = other.m_id;
    ...
    ApplicationComponentDTO &mergeWith(const ApplicationComponentDTO &other)
    {
        if (this != &other) {
    ...
    friend bool operator==(const ApplicationComponentDTO &lhs, const ApplicationComponentDTO &rhs);

    friend uint qHash(const ApplicationComponentDTO &dto, uint seed) noexcept;
...
inline bool operator==(const ApplicationComponentDTO &lhs, const ApplicationComponentDTO &rhs)
{
...
inline uint qHash(const ApplicationComponentDTO &dto, uint seed = 0) noexcept
...
Q_DECLARE_METATYPE(QleanyEditor::Contracts::DTO::ApplicationComponent::ApplicationComponentDTO)