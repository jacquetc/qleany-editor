...
class ApplicationFeatureRelationDTO
{
    Q_GADGET
...
public:
    ...
    enum RelationField { Undefined, DtoIdenticalToEntity, Crud, CustomCommands, CustomQueries };
    Q_ENUM(RelationField);

    ApplicationFeatureRelationDTO()
        : m_id(0)
    ...
    ApplicationFeatureRelationDTO(int id, const RelationField &relationField, const QList<int> &relatedIds, int position)
        : m_id(id)
    ...
    ApplicationFeatureRelationDTO(const ApplicationFeatureRelationDTO &other)
    ...
    ApplicationFeatureRelationDTO &operator=(const ApplicationFeatureRelationDTO &other)
    {
        if (this != &other) {
            m_metaData = other.m_metaData;
            m_id = other.m_id;
    ...
    ApplicationFeatureRelationDTO &mergeWith(const ApplicationFeatureRelationDTO &other)
    {
        if (this != &other) {
    ...
    friend bool operator==(const ApplicationFeatureRelationDTO &lhs, const ApplicationFeatureRelationDTO &rhs);

    friend uint qHash(const ApplicationFeatureRelationDTO &dto, uint seed) noexcept;
...
inline bool operator==(const ApplicationFeatureRelationDTO &lhs, const ApplicationFeatureRelationDTO &rhs)
{
...
inline uint qHash(const ApplicationFeatureRelationDTO &dto, uint seed = 0) noexcept
...
Q_DECLARE_METATYPE(QleanyEditor::Contracts::DTO::ApplicationFeature::ApplicationFeatureRelationDTO)