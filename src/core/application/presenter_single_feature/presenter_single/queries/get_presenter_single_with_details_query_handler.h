...
{
class QLEANY_EDITOR_APPLICATION_PRESENTER_SINGLE_EXPORT GetPresenterSingleWithDetailsQueryHandler : public QObject
{
    Q_OBJECT
public:
    GetPresenterSingleWithDetailsQueryHandler(InterfacePresenterSingleRepository *repository);
    Result<PresenterSingleWithDetailsDTO> handle(QPromise<Result<void>> &progressPromise, const GetPresenterSingleQuery &query);

private:
    InterfacePresenterSingleRepository *m_repository;
    Result<PresenterSingleWithDetailsDTO> handleImpl(QPromise<Result<void>> &progressPromise, const GetPresenterSingleQuery &query);
