...
class EntityComponentRelationDTO
{
    Q_GADGET
...
public:
    ...
    enum RelationField { Undefined, Entities };
    Q_ENUM(RelationField);

    EntityComponentRelationDTO()
        : m_id(0)
    ...
    EntityComponentRelationDTO(int id, const RelationField &relationField, const QList<int> &relatedIds, int position)
        : m_id(id)
    ...
    EntityComponentRelationDTO(const EntityComponentRelationDTO &other)
    ...
    EntityComponentRelationDTO &operator=(const EntityComponentRelationDTO &other)
    {
        if (this != &other) {
            m_metaData = other.m_metaData;
            m_id = other.m_id;
    ...
    EntityComponentRelationDTO &mergeWith(const EntityComponentRelationDTO &other)
    {
        if (this != &other) {
    ...
    friend bool operator==(const EntityComponentRelationDTO &lhs, const EntityComponentRelationDTO &rhs);

    friend uint qHash(const EntityComponentRelationDTO &dto, uint seed) noexcept;
...
inline bool operator==(const EntityComponentRelationDTO &lhs, const EntityComponentRelationDTO &rhs)
{
...
inline uint qHash(const EntityComponentRelationDTO &dto, uint seed = 0) noexcept
...
Q_DECLARE_METATYPE(QleanyEditor::Contracts::DTO::EntityComponent::EntityComponentRelationDTO)