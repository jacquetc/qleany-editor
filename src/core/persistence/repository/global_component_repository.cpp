...
#include "global_component_repository.h"
#ifdef QT_DEBUG
#include <QDebug>
#include <QObject>
#endif
...
GlobalComponentRepository::GlobalComponentRepository(InterfaceDatabaseTableGroup<QleanyEditor::Entities::GlobalComponent> *globalComponentDatabase)
    : Qleany::Repository::GenericRepository<QleanyEditor::Entities::GlobalComponent>(globalComponentDatabase)
{
    m_signalHolder.reset(new SignalHolder(nullptr));
}

SignalHolder *GlobalComponentRepository::signalHolder()
{
    QReadLocker locker(&m_lock);
    return m_signalHolder.data();
}

Result<QHash<int, QList<int>>> GlobalComponentRepository::removeInCascade(QList<int> ids)
{
    ...
    returnedHashOfEntityWithRemovedIds.insert(QleanyEditor::Entities::Entities::GlobalComponent, removedIdsResult.value());
...
Result<QHash<int, QList<int>>> GlobalComponentRepository::changeActiveStatusInCascade(QList<int> ids, bool active)
{
    QWriteLocker locker(&m_lock);
    ...
    returnedHashOfEntityWithActiveChangedIds.insert(QleanyEditor::Entities::Entities::GlobalComponent, changedIdsResult.value());
