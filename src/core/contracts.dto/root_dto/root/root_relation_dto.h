...
public:
    ...
    enum RelationField { Undefined, RecentProjects, Project };
    ...
    RootRelationDTO(int id, const RelationField &relationField, const QList<int> &relatedIds, int position)
        : m_id(id)
    ...
    RootRelationDTO(const RootRelationDTO &other)
    ...
    RootRelationDTO &operator=(const RootRelationDTO &other)
    {
        if (this != &other) {
            m_metaData = other.m_metaData;
            m_id = other.m_id;
    ...
    RootRelationDTO &mergeWith(const RootRelationDTO &other)
    {
        if (this != &other) {
    ...
    friend bool operator==(const RootRelationDTO &lhs, const RootRelationDTO &rhs);

    friend uint qHash(const RootRelationDTO &dto, uint seed) noexcept;
...
inline bool operator==(const RootRelationDTO &lhs, const RootRelationDTO &rhs)
{
...
inline uint qHash(const RootRelationDTO &dto, uint seed = 0) noexcept
...
Q_DECLARE_METATYPE(QleanyEditor::Contracts::DTO::Root::RootRelationDTO)