...
public:
    ...
    CustomQueryRelationDTO(int id, const RelationField &relationField, const QList<int> &relatedIds, int position)
        : m_id(id)
    ...
    CustomQueryRelationDTO(const CustomQueryRelationDTO &other)
    ...
    CustomQueryRelationDTO &operator=(const CustomQueryRelationDTO &other)
    {
        if (this != &other) {
            m_metaData = other.m_metaData;
            m_id = other.m_id;
    ...
    CustomQueryRelationDTO &mergeWith(const CustomQueryRelationDTO &other)
    {
        if (this != &other) {
    ...
    friend bool operator==(const CustomQueryRelationDTO &lhs, const CustomQueryRelationDTO &rhs);

    friend uint qHash(const CustomQueryRelationDTO &dto, uint seed) noexcept;
...
inline bool operator==(const CustomQueryRelationDTO &lhs, const CustomQueryRelationDTO &rhs)
{
...
inline uint qHash(const CustomQueryRelationDTO &dto, uint seed = 0) noexcept
...
Q_DECLARE_METATYPE(QleanyEditor::Contracts::DTO::CustomQuery::CustomQueryRelationDTO)