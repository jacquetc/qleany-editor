...
public:
    ...
    RepositoryRelationDTO(int id, const RelationField &relationField, const QList<int> &relatedIds, int position)
        : m_id(id)
    ...
    RepositoryRelationDTO(const RepositoryRelationDTO &other)
    ...
    RepositoryRelationDTO &operator=(const RepositoryRelationDTO &other)
    {
        if (this != &other) {
            m_metaData = other.m_metaData;
            m_id = other.m_id;
    ...
    RepositoryRelationDTO &mergeWith(const RepositoryRelationDTO &other)
    {
        if (this != &other) {
    ...
    friend bool operator==(const RepositoryRelationDTO &lhs, const RepositoryRelationDTO &rhs);

    friend uint qHash(const RepositoryRelationDTO &dto, uint seed) noexcept;
...
inline bool operator==(const RepositoryRelationDTO &lhs, const RepositoryRelationDTO &rhs)
{
...
inline uint qHash(const RepositoryRelationDTO &dto, uint seed = 0) noexcept
...
Q_DECLARE_METATYPE(QleanyEditor::Contracts::DTO::Repository::RepositoryRelationDTO)