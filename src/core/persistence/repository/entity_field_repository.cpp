...
#include "entity_field_repository.h"
#ifdef QT_DEBUG
#include <QDebug>
#include <QObject>
#endif
...
EntityFieldRepository::EntityFieldRepository(InterfaceDatabaseTableGroup<QleanyEditor::Entities::EntityField> *entityFieldDatabase)
    : Qleany::Repository::GenericRepository<QleanyEditor::Entities::EntityField>(entityFieldDatabase)
{
    m_signalHolder.reset(new SignalHolder(nullptr));
}

SignalHolder *EntityFieldRepository::signalHolder()
{
    QReadLocker locker(&m_lock);
    return m_signalHolder.data();
}

Result<QHash<int, QList<int>>> EntityFieldRepository::removeInCascade(QList<int> ids)
{
    ...
    returnedHashOfEntityWithRemovedIds.insert(QleanyEditor::Entities::Entities::EntityField, removedIdsResult.value());
...
Result<QHash<int, QList<int>>> EntityFieldRepository::changeActiveStatusInCascade(QList<int> ids, bool active)
{
    QWriteLocker locker(&m_lock);
    ...
    returnedHashOfEntityWithActiveChangedIds.insert(QleanyEditor::Entities::Entities::EntityField, changedIdsResult.value());
