...
public:
    ...
    enum RelationField { Undefined, Parent, Fields };
    ...
    EntityRelationDTO(int id, const RelationField &relationField, const QList<int> &relatedIds, int position)
        : m_id(id)
    ...
    EntityRelationDTO(const EntityRelationDTO &other)
    ...
    EntityRelationDTO &operator=(const EntityRelationDTO &other)
    {
        if (this != &other) {
            m_metaData = other.m_metaData;
            m_id = other.m_id;
    ...
    EntityRelationDTO &mergeWith(const EntityRelationDTO &other)
    {
        if (this != &other) {
    ...
    friend bool operator==(const EntityRelationDTO &lhs, const EntityRelationDTO &rhs);

    friend uint qHash(const EntityRelationDTO &dto, uint seed) noexcept;
...
inline bool operator==(const EntityRelationDTO &lhs, const EntityRelationDTO &rhs)
{
...
inline uint qHash(const EntityRelationDTO &dto, uint seed = 0) noexcept
...
Q_DECLARE_METATYPE(QleanyEditor::Contracts::DTO::Entity::EntityRelationDTO)