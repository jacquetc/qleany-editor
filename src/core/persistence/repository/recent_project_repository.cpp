...
#include "recent_project_repository.h"
#ifdef QT_DEBUG
#include <QDebug>
#include <QObject>
#endif
...
RecentProjectRepository::RecentProjectRepository(InterfaceDatabaseTableGroup<QleanyEditor::Entities::RecentProject> *recentProjectDatabase)
    : Qleany::Repository::GenericRepository<QleanyEditor::Entities::RecentProject>(recentProjectDatabase)
{
    m_signalHolder.reset(new SignalHolder(nullptr));
}

SignalHolder *RecentProjectRepository::signalHolder()
{
    QReadLocker locker(&m_lock);
    return m_signalHolder.data();
}

Result<QHash<int, QList<int>>> RecentProjectRepository::removeInCascade(QList<int> ids)
{
    ...
    returnedHashOfEntityWithRemovedIds.insert(QleanyEditor::Entities::Entities::RecentProject, removedIdsResult.value());
...
Result<QHash<int, QList<int>>> RecentProjectRepository::changeActiveStatusInCascade(QList<int> ids, bool active)
{
    QWriteLocker locker(&m_lock);
    ...
    returnedHashOfEntityWithActiveChangedIds.insert(QleanyEditor::Entities::Entities::RecentProject, changedIdsResult.value());
