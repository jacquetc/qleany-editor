...
#include "d_t_o_component_repository.h"
#ifdef QT_DEBUG
#include <QDebug>
#include <QObject>
#endif
...
DTOComponentRepository::DTOComponentRepository(InterfaceDatabaseTableGroup<QleanyEditor::Entities::DTOComponent> *dTOComponentDatabase)
    : Qleany::Repository::GenericRepository<QleanyEditor::Entities::DTOComponent>(dTOComponentDatabase)
{
    m_signalHolder.reset(new SignalHolder(nullptr));
}

SignalHolder *DTOComponentRepository::signalHolder()
{
    QReadLocker locker(&m_lock);
    return m_signalHolder.data();
}

Result<QHash<int, QList<int>>> DTOComponentRepository::removeInCascade(QList<int> ids)
{
    ...
    returnedHashOfEntityWithRemovedIds.insert(QleanyEditor::Entities::Entities::DTOComponent, removedIdsResult.value());
...
Result<QHash<int, QList<int>>> DTOComponentRepository::changeActiveStatusInCascade(QList<int> ids, bool active)
{
    QWriteLocker locker(&m_lock);
    ...
    returnedHashOfEntityWithActiveChangedIds.insert(QleanyEditor::Entities::Entities::DTOComponent, changedIdsResult.value());
