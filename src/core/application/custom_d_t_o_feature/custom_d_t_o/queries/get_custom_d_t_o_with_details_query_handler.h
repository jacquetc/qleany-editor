...
{
class QLEANY_EDITOR_APPLICATION_CUSTOM_D_T_O_EXPORT GetCustomDTOWithDetailsQueryHandler : public QObject
{
    Q_OBJECT
public:
    GetCustomDTOWithDetailsQueryHandler(InterfaceCustomDTORepository *repository);
    Result<CustomDTOWithDetailsDTO> handle(QPromise<Result<void>> &progressPromise, const GetCustomDTOQuery &query);

private:
    InterfaceCustomDTORepository *m_repository;
    Result<CustomDTOWithDetailsDTO> handleImpl(QPromise<Result<void>> &progressPromise, const GetCustomDTOQuery &query);
