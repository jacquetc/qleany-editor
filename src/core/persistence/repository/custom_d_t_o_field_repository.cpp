...
#include "custom_d_t_o_field_repository.h"
#ifdef QT_DEBUG
#include <QDebug>
#include <QObject>
#endif
...
CustomDTOFieldRepository::CustomDTOFieldRepository(InterfaceDatabaseTableGroup<QleanyEditor::Entities::CustomDTOField> *customDTOFieldDatabase)
    : Qleany::Repository::GenericRepository<QleanyEditor::Entities::CustomDTOField>(customDTOFieldDatabase)
{
    m_signalHolder.reset(new SignalHolder(nullptr));
}

SignalHolder *CustomDTOFieldRepository::signalHolder()
{
    QReadLocker locker(&m_lock);
    return m_signalHolder.data();
}

Result<QHash<int, QList<int>>> CustomDTOFieldRepository::removeInCascade(QList<int> ids)
{
    ...
    returnedHashOfEntityWithRemovedIds.insert(QleanyEditor::Entities::Entities::CustomDTOField, removedIdsResult.value());
...
Result<QHash<int, QList<int>>> CustomDTOFieldRepository::changeActiveStatusInCascade(QList<int> ids, bool active)
{
    QWriteLocker locker(&m_lock);
    ...
    returnedHashOfEntityWithActiveChangedIds.insert(QleanyEditor::Entities::Entities::CustomDTOField, changedIdsResult.value());
