...
#include "interactor_component_repository.h"
#ifdef QT_DEBUG
#include <QDebug>
#include <QObject>
#endif
...
InteractorComponentRepository::InteractorComponentRepository(
    InterfaceDatabaseTableGroup<QleanyEditor::Entities::InteractorComponent> *interactorComponentDatabase)
    : Qleany::Repository::GenericRepository<QleanyEditor::Entities::InteractorComponent>(interactorComponentDatabase)
{
    m_signalHolder.reset(new SignalHolder(nullptr));
}

SignalHolder *InteractorComponentRepository::signalHolder()
{
    QReadLocker locker(&m_lock);
    return m_signalHolder.data();
}

Result<QHash<int, QList<int>>> InteractorComponentRepository::removeInCascade(QList<int> ids)
{
    ...
    returnedHashOfEntityWithRemovedIds.insert(QleanyEditor::Entities::Entities::InteractorComponent, removedIdsResult.value());
...
Result<QHash<int, QList<int>>> InteractorComponentRepository::changeActiveStatusInCascade(QList<int> ids, bool active)
{
    QWriteLocker locker(&m_lock);
    ...
    returnedHashOfEntityWithActiveChangedIds.insert(QleanyEditor::Entities::Entities::InteractorComponent, changedIdsResult.value());
