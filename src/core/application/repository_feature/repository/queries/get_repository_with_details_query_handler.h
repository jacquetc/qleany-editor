...
{
class QLEANY_EDITOR_APPLICATION_REPOSITORY_EXPORT GetRepositoryWithDetailsQueryHandler : public QObject
{
    Q_OBJECT
public:
    GetRepositoryWithDetailsQueryHandler(InterfaceRepositoryRepository *repository);
    Result<RepositoryWithDetailsDTO> handle(QPromise<Result<void>> &progressPromise, const GetRepositoryQuery &query);

private:
    InterfaceRepositoryRepository *m_repository;
    Result<RepositoryWithDetailsDTO> handleImpl(QPromise<Result<void>> &progressPromise, const GetRepositoryQuery &query);
