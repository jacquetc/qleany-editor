...
{
class QLEANY_EDITOR_APPLICATION_PRESENTER_LIST_MODEL_EXPORT GetPresenterListModelQueryHandler : public QObject
{
    Q_OBJECT
public:
    GetPresenterListModelQueryHandler(InterfacePresenterListModelRepository *repository);
    Result<PresenterListModelDTO> handle(QPromise<Result<void>> &progressPromise, const GetPresenterListModelQuery &query);

private:
    InterfacePresenterListModelRepository *m_repository;
    Result<PresenterListModelDTO> handleImpl(QPromise<Result<void>> &progressPromise, const GetPresenterListModelQuery &query);
