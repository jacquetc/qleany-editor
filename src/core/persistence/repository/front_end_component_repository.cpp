...
#include "front_end_component_repository.h"
#ifdef QT_DEBUG
#include <QDebug>
#include <QObject>
#endif
...
FrontEndComponentRepository::FrontEndComponentRepository(InterfaceDatabaseTableGroup<QleanyEditor::Entities::FrontEndComponent> *frontEndComponentDatabase)
    : Qleany::Repository::GenericRepository<QleanyEditor::Entities::FrontEndComponent>(frontEndComponentDatabase)
{
    m_signalHolder.reset(new SignalHolder(nullptr));
}

SignalHolder *FrontEndComponentRepository::signalHolder()
{
    QReadLocker locker(&m_lock);
    return m_signalHolder.data();
}

Result<QHash<int, QList<int>>> FrontEndComponentRepository::removeInCascade(QList<int> ids)
{
    ...
    returnedHashOfEntityWithRemovedIds.insert(QleanyEditor::Entities::Entities::FrontEndComponent, removedIdsResult.value());
...
Result<QHash<int, QList<int>>> FrontEndComponentRepository::changeActiveStatusInCascade(QList<int> ids, bool active)
{
    QWriteLocker locker(&m_lock);
    ...
    returnedHashOfEntityWithActiveChangedIds.insert(QleanyEditor::Entities::Entities::FrontEndComponent, changedIdsResult.value());
