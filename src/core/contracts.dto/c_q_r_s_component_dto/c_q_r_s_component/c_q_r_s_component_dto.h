...
public:
    ...
    CQRSComponentDTO(int id, const QUuid &uuid, const QDateTime &creationDate, const QDateTime &updateDate, const QString &commonCMakeFolderPath)
        : m_id(id)
        , m_uuid(uuid)
    ...
    CQRSComponentDTO(const CQRSComponentDTO &other)
    ...
    CQRSComponentDTO &operator=(const CQRSComponentDTO &other)
    {
        if (this != &other) {
            m_metaData = other.m_metaData;
            m_id = other.m_id;
    ...
    CQRSComponentDTO &mergeWith(const CQRSComponentDTO &other)
    {
        if (this != &other) {
    ...
    friend bool operator==(const CQRSComponentDTO &lhs, const CQRSComponentDTO &rhs);

    friend uint qHash(const CQRSComponentDTO &dto, uint seed) noexcept;
...
inline bool operator==(const CQRSComponentDTO &lhs, const CQRSComponentDTO &rhs)
{
...
inline uint qHash(const CQRSComponentDTO &dto, uint seed = 0) noexcept
...
Q_DECLARE_METATYPE(QleanyEditor::Contracts::DTO::CQRSComponent::CQRSComponentDTO)