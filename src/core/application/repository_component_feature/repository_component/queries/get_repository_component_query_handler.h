...
{
class QLEANY_EDITOR_APPLICATION_REPOSITORY_COMPONENT_EXPORT GetRepositoryComponentQueryHandler : public QObject
{
    Q_OBJECT
public:
    GetRepositoryComponentQueryHandler(InterfaceRepositoryComponentRepository *repository);
    Result<RepositoryComponentDTO> handle(QPromise<Result<void>> &progressPromise, const GetRepositoryComponentQuery &query);

private:
    InterfaceRepositoryComponentRepository *m_repository;
    Result<RepositoryComponentDTO> handleImpl(QPromise<Result<void>> &progressPromise, const GetRepositoryComponentQuery &query);
