...
public:
    ...
    CustomQueryDTO(int id, const QUuid &uuid, const QDateTime &creationDate, const QDateTime &updateDate, const QString &name, bool validator, bool undo)
        : m_id(id)
        , m_uuid(uuid)
    ...
    CustomQueryDTO(const CustomQueryDTO &other)
    ...
    CustomQueryDTO &operator=(const CustomQueryDTO &other)
    {
        if (this != &other) {
            m_metaData = other.m_metaData;
            m_id = other.m_id;
    ...
    CustomQueryDTO &mergeWith(const CustomQueryDTO &other)
    {
        if (this != &other) {
    ...
    friend bool operator==(const CustomQueryDTO &lhs, const CustomQueryDTO &rhs);

    friend uint qHash(const CustomQueryDTO &dto, uint seed) noexcept;
...
inline bool operator==(const CustomQueryDTO &lhs, const CustomQueryDTO &rhs)
{
...
inline uint qHash(const CustomQueryDTO &dto, uint seed = 0) noexcept
...
Q_DECLARE_METATYPE(QleanyEditor::Contracts::DTO::CustomQuery::CustomQueryDTO)